...
function Dashboard({ activeTab }) {
    ...
    const fetchStats   = () => fetch(`${BACKEND}/analytics`).then(r => r.json()).then(setStats).catch(() => {});
    const fetchServers = () => fetch(`${BACKEND}/servers`).then(r => r.json()).then(setServers).catch(() => {});
    ...
    useEffect(() => {
        ...
        socket.on("requestUpdate", data => setRequests(prev => [data, ...prev].slice(0, 20)));
        return () => {
    ...
    const runDemo = async () => {
        ...
        setDemoMsg("🎉 Demo complete! Check the table below.");
        setRunning(false);
    };
                ...
                <div className="charts-grid" style={{ marginTop:32 }}>
                    ...
                    <div className="chart-card">
                        ...
                        <div className="cache-info">
                            <div className="cache-row"><span>🔵 Redis (Primary)</span>       <span className="online">● Active</span></div>
                            <div className="cache-row"><span>🟡 Memory Cache (Fallback)</span><span className="online">● Active</span></div>
                            <div className="cache-row"><span>⏱ Default TTL</span>            <span>300 seconds</span></div>
                            <div className="cache-row"><span>📦 Cached Keys</span>            <span>{stats.originRequests} unique routes</span></div>
                            <div className="cache-row"><span>⚡ Avg Hit Speed</span>          <span style={{color:"#16a34a"}}>~2–5 ms</span></div>
                            <div className="cache-row"><span>🌐 Avg Miss Speed</span>         <span style={{color:"#ef4444"}}>~800–1500 ms</span></div>
                        </div>
